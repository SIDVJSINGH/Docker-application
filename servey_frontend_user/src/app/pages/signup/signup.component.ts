...
export class SignupComponent implements OnInit, OnDestroy {
  ...
  emailOtp() {
    ...
    if (this.form.valid) {
        ...
        (response: any) => {
          if (response.status == true) {
            ...
            this.webservice.notifySuccess(
              'We have sent you a six digit One Time Password (OTP) to your registered email address'
            );
          } else {
            this.timerSubscription.unsubscribe();
      ...
      this.webservice.notifyError1(
        'Please fill out all the required fields and age should be above 16.'
      );
    }
  }
      ...
      this.webservice.sendPhone(payload1).subscribe(
        (response: any) => {
          if (response.status == true) {
            ...
            this.webservice.notifySuccess(
              'We have sent you a six digit One Time Password (OTP)'
            );
          } else {
            // this.showPhoneOtp = false;
            this.timerSubscription.unsubscribe();
        ...
        (errorResponse) => {
          ...
          if (errorResponse.status) {
            ...
            this.webservice.notifyError('Phone is not allowed to be empty');
          } else {
            this.webservice.notifyError('Phone is not allowed to be empty');
          }
        }
